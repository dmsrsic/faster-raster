...
def test_range_sources_plain():
    ...
    assert "fixture_only" not in output
...
def test_range_plan_dry_run_writes_reports(monkeypatch, tmp_path):
    ...
    assert "runnable_source_count: 5" in output
    assert "fixture_source_count: 0" in output
    assert "network_run: False" in output
    assert (
        static_http_range.DEFAULT_REPORT_DIR
        / "static_http_range_wave1_plan.json"
    ).exists()
...
def test_range_probe_prism_dry_run_no_network(monkeypatch):
    def fail_network(*args, **kwargs):
        raise AssertionError("network access attempted")
...
def test_task_show_reports_static_range_availability(tmp_path, monkeypatch):
    ...
    payload = json.loads(
        ok(["task", "show", "range_task", "--json"])
    )
    assert payload["static_range_wave1_available_sources"] == [
        "chirps_daily_precipitation"
    ]
