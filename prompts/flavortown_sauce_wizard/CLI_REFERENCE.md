...
`sauce validate`, `sauce explain`, `sauce test`, Sauce Time fixture discovery,
and preview-template discovery are offline. `sauce probe` is the only Source
Pack command above that may use network; it requires explicit authorization
and is bounded by the pack's request, byte, timeout, host, and redirect limits.
